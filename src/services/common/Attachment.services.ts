...
class AttachmentService {
  ...
  static async uploadAttachment(formData: FormData) {
    return HttpService.callApi( API_ENDPOINTS.ATTACHMENT.UPLOAD, "POST", formData, false,true );
}
 
static async downloadAttachment(attachmentId: number, fileName: string) {
  return HttpService.downloadFile(API_ENDPOINTS.ATTACHMENT.DOWNLOAD(attachmentId),
    fileName
  );
}
